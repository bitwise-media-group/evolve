...
// preselected by a --new/--failed sweep. ReasonNone means the stored result is
// complete and the case is not selected. The same value decides both what the
// engine reruns and what the TUI form annotates, so the two never diverge:
// a (case, model) runs exactly when its reason is not ReasonNone.
type SelectReason int

const (
	ReasonNone                SelectReason = iota // complete; not selected
	ReasonNew                                     // no stored result for this model
	ReasonModified                                // authored should_trigger changed (triggers only)
...
// model, or ReasonNone when its stored result is complete. Token-count
// estimates are intentionally not a reason: the TUI cannot probe them cheaply,
// so keeping them out keeps CLI and TUI selection identical (the count still
// refreshes whenever the case runs for another reason).
func triggerCaseReason(r results.TriggerResult, ok bool, t evalspec.Trigger,
	execute, wantNew, wantFailed bool) SelectReason {
	...
	if wantNew {
		...
		if r.ShouldTrigger != t.ShouldTrigger {
...
// ReasonNone when complete. Like triggerCaseReason it ignores token-count
// estimates; the measured-usage reasons cover the fields a real run produces.
func evalCaseReason(r results.EvalResult, ok bool,
	execute, reportsUsage, priced, wantNew, wantFailed bool) SelectReason {
...
func lookupTrigger(file *results.File, key, query string) (results.TriggerResult, bool) {
	if file == nil {
...
func lookupEval(file *results.File, key, id string) (results.EvalResult, bool) {
	if file == nil {

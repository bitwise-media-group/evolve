...
//
// active is the model restriction (a configured `models` set), the same map
// passed to Generate as Options.ActiveModels: non-nil restricts the gate to
// those models. It is applied to the loaded files so the strict staleness path
// (which iterates every stored model) matches the pass-rate path (which reads
// the already-active-filtered summary): a stale leftover result for an excluded
// model must not FAIL a gated plugin. Nil means no restriction (every stored
// model participates).
func Check(repo *layout.Repo, summary *Summary, th Thresholds, active map[string]bool) (fails, warns []string) {
	...
	// Match Generate: when a `models` restriction is active, drop excluded models
	// from the loaded files so the staleness path only inspects active evidence.
	if active != nil {
		filterActive(loaded, active)
	}
	...
	// Drive the loop from the authored eval sets, not summary.Plugins: a plugin
	// with authored evals but no stored results never reaches summary.Plugins
	// (Generate skips empty rollups), yet its absent evidence must still gate.
	// pluginIssues tolerates a nil summary, so an unevidenced plugin surfaces its
	// missing-results issues here rather than passing silently.
	for _, name := range sortedKeys(setsByPlugin) {
		sets := setsByPlugin[name]
		ps := summary.Plugins[name] // nil when the plugin has no stored results
		m := pluginMaturity(sets)
		issues := pluginIssues(name, ps, th, sets, filesByPlugin[name])
		if th.gated(m) {
...
// pluginMaturity classifies a plugin from its detected directory, taken off its
// authored eval sets (every set of a plugin carries the same Plugin.Dir). It
// reads the manifest only from that absolute directory — never a cwd-relative
// path — so an empty or manifest-less directory classifies as MaturityUnknown
// (never gated) instead of parsing whatever `.claude-plugin/plugin.json` happens
// to sit in the process's working directory.
func pluginMaturity(sets []layout.EvalSet) Maturity {
	if len(sets) == 0 || sets[0].Plugin.Dir == "" {
		return MaturityUnknown
	}
	return Classify(manifest.PluginVersion(sets[0].Plugin.Dir))
}

// pluginIssues computes one plugin's evidence-gate issues, each prefixed with
// the plugin name so a marketplace's two plugins missing the same model emit
// distinct, attributable lines. The below-threshold pass-rate check runs for any
// --check, and no-stored-results fires whenever the model matrix is pinned
// (--strict's Defined set or report.thresholds.models) — both scoped from the
// pre-maturity aggregate down to a single plugin. The staleness check (reusing
// run.StaleTiers, the primitive --modified uses) is the strictest requirement and
// runs only under --strict, so a plain --check stays a pass-rate gate; its
// per-skill messages already name the skill, so they carry no plugin prefix.
func pluginIssues(
	plugin string, ps *PluginSummary, th Thresholds, sets []layout.EvalSet, files []*results.File,
) []string {
	...
	// A tier is only gated for a plugin that authored it: triggers-only and
	// evals-only skills are first-class (layout admits a set when either tier is
	// present), so a plugin that never authored a tier must not emit a spurious
	// "<tier>: no stored results" breach for it under --strict or pinned models.
	// This is plugin-level presence (any skill authored the tier); run.StaleTiers
	// makes the same TriggersPath/EvalsPath check per skill, a deliberately
	// different granularity (per-skill staleness), so the two are not shared.
	var authoredTriggers, authoredEvals bool
	for _, set := range sets {
		authoredTriggers = authoredTriggers || set.TriggersPath != ""
		authoredEvals = authoredEvals || set.EvalsPath != ""
	}

	var issues []string
	check := func(kind string, minRate float64, authored bool) {
		if !authored {
			return
		}
		keys := th.Models
		...
		for _, key := range keys {
			...
			if a == nil || a.executed == 0 {
				issues = append(issues, fmt.Sprintf("%s: %s: no stored results for %s", plugin, kind, key))
				continue
			}
			rate := float64(a.passed) / float64(a.executed)
			if rate < minRate {
				issues = append(issues, fmt.Sprintf("%s: %s: %s pass rate %.1f%% below threshold %.1f%%",
					plugin, kind, key, rate*100, minRate*100))
			}
		}
	}
	check("triggers", th.TriggersMinPassRate, authoredTriggers)
	check("evals", th.EvalsMinPassRate, authoredEvals)
	if th.Strict {
